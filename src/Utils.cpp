...
namespace Utils {
	...
	void handleCBFDL() {
		...
		if (cbf && getBool("hideInLevelEditorLayer") && lel) return cbf->setVisible(false);
		...
		if (gjbgl) {
			...
			if (pl) {
				...
				if (getBool("hideInLevelEditorLayer")) return cbf->setVisible(false);
				else addCBFDL();
			}
		} else {
			resultString = resultString.append("in GD.");
			if (getBool("hideEverywhereElse")) return cbf->setVisible(false);
			else addCBFDL();
		}
